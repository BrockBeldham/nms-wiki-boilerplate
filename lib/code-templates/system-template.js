export default function systemTemplate(data) {
  const renderPlanets = () => {
    const usedResources = [];
    return data.planets.map((planet) => planet ? `
| [[File:${planet.photo || 'nmsMisc_NotAvailable.png'}|150px]] 
...
| economybuy = ${data.economybuy ? `${data.economybuy.replace(/[^\d.-]/g, '')}%` : ''}
| economysell = ${data.economysell ? `${data.economysell.replace(/[^\d.-]/g, '')}%` : ''}
| wealth = ${data.wealth}
...
===Glyphs===
${data.glyphs ? `{{Gl|${data.glyphs}}}` : ''}

===Navigation Images===
${data.galaxyMap ? `[[File:${data.galaxyMap}|400px]]`: ''}
