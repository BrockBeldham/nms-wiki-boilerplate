...
It is located in the [[${data.region || 'RegionName'}]] region of the [[${data.galaxy || 'GalaxyName'}]] galaxy.
...
==Location==
===Headquarters===
Company headquarters are located in the ${data.region || 'RegionName'} region of the ${data.galaxy || 'GalaxyName'} galaxy.

===Regions===
${data.region ? `Company branch locations can be found in these regions:
* [[${data.region}]]\n` : 'Company branch locations can be found in these regions:'}${data.regions.length > 0 && data.regions[0] !== '' ? data.regions.map((region) => `* [[${region}]]\n`).join('') : ''}

===Branch Offices===
${data.title} has the following branches:
...
==Partnerships==
${data.partnerships.length > 0 && data.partnerships[0] !== '' ? `Other companies and [[Civilized space|civilizations]] that share a preferred partnership:
