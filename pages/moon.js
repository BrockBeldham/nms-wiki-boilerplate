...
import moonTemplate from '../lib/code-templates/moon-template';
...
export default function System() {
  ...
  const codeTemplate = moonTemplate(data);
    ...
    >
      ...
      <div className={styles.btnContainer}>
        <button type='button' className={`btn ${styles.btn}`} onClick={() => {
          ...
          setViewCode(true);
          ga.event('View Code', 'Moon', window.innerWidth < 800 ? 'Popup' : 'ScrollTo');
        }}>
          View Code
        </button>
        <button type='button' className={`btn ${styles.btn}`} onClick={() => {
          handleCopy(codeTemplate);
          ga.event('Copy Code', 'Moon', data.civ);
        }}>
        ...
        <a href={`https://nomanssky.fandom.com/wiki/${data.title.replace(/ /g,'_')}?action=edit`}
          ...
          target='_blank'
          rel='noreferrer'
          onClick={() => ga.event('Create Page', 'Moon', data.civ)}>
          Create Page
        </a>
      </div>
